...
describe('일간 랭킹 생성 함수 테스트', () => {
    ...
    it("통합 점수 랭킹을 산정하는 함수가 정상적으로 동작한다.", async () => {
        ...
        sandbox.stub(lambda, "getContentSummary").returns(summary);
    ...
    it("랭킹에 등락정보가 정상적으로 생성된다.", async () => {
        ...
        sandbox.stub(lambda, "getBeforeRankingDt").returns({Items: [{target_dt: targetDate}]});
    ...
    it("랭킹정보로 올바른 포맷의 이벤트 메시지가 생성된다", async () => {
        ...
        rankingMap = await lambda.setRankAndUpDownInfo(targetDate, region, language, rankingMap);
        let genreCode = "ALL"
        let domainEvent = JSON.parse(await lambda.makeDomainEvent(region, language, genreCode, JSON.parse(rankingMap.get(genreCode))));
        ...
        genreCode = "DRAMA"
        domainEvent = JSON.parse(await lambda.makeDomainEvent(region, language, genreCode, JSON.parse(rankingMap.get(genreCode))));
        expect(domainEvent.event.type).to.be.eq("CREATE");
        expect(domainEvent.event.attributes[0].region).to.be.eq(region);
        expect(domainEvent.event.attributes[0].language).to.be.eq(language);
        expect(domainEvent.event.attributes[0].list.length).to.be.eq(1);
