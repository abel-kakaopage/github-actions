...
async function getGroupInfos(targetDt) {
    const query = `SELECT region, user_language
                   FROM 
                        "${process.env.ATHENA_DB}".stat_content_summary_weekly
                   WHERE 
...
async function splitInsertToAthena(targetDt, uuid, rankingChart) {
    if (rankingChart) {
        ...
        rankingChart.forEach(function (item) {
            insertDatas.push(`('${uuid}',${item.content_id},'${item.genre_code}',${item.rank},${item.rank_last},'WEEKLY-${targetDt}')`);
        })
...
async function existChart(targetDt) {
    let query = `
            ...
            AND chart.target_dt='WEEKLY-${targetDt}'
            AND info.type='WEEKLY'
            LIMIT 1`;
...
async function getRankingChart(region, language) {
    const query = `
            SELECT chart.content_id, chart.rank 
            FROM "${process.env.ATHENA_DB}".ranking_chart chart
            INNER JOIN "${process.env.ATHENA_DB}".ranking_info info ON chart.uuid = info.uuid
            WHERE info.target_dt=(SELECT max(target_dt) FROM "${process.env.ATHENA_DB}".ranking_info WHERE type='WEEKLY')
            AND info.type='WEEKLY'
...
// 랭킹 정보에 등락정보 추가
async function setRankAndUpDownInfo(region, language, totalRankings) {
    // 이전 랭킹 정보와 비교하여 등락 정보를 생성
    const lastRanking = await this.getRankingChart(region, language)
...
async function produceKafkaMessage(message) {
    try {
        ...
        const kafka = new Kafka({
            clientId: 'generate-weekly-ranking',
...
async function showPartition(table) {
    let query = `SHOW PARTITIONS ${process.env.ATHENA_DB}.${table}`;
    let partitions = [];
    try {
        await athenaExpress.query(query).then(function (data) {
            if (data.Items) {
                data.Items.forEach(function (item) {
                    partitions.push(item['row']);
                });
            }
        })
        return partitions;
    } catch (error) {
        console.error('[ERROR] showPartition', error);
    }
}

async function createPartitions(table) {
    const partitions = await showPartition(table);
    let params = {
        Bucket: process.env.S3_BUCKET_NAME,
        Prefix: table
    }
