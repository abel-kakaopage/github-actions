...
describe('주간 랭킹 생성 함수 테스트', () => {
    ...
    it("랭킹에 등락정보가 정상적으로 생성된다.", async () => {
        ...
        const rankings = await ranking.setRankAndUpDownInfo(region, language, totalRanking);
    ...
    it("랭킹정보로 올바른 포맷의 이벤트 메시지가 생성된다", async () => {
        ...
        sandbox.stub(ranking, "getRankingChart").returns(lastRanking);
        let rankings = await ranking.setRankAndUpDownInfo(region, language,  totalRankings);
        ...
        expect(domainEvent.event.attributes[0].list[0].type).to.be.eq("WEEKLY");
        expect(domainEvent.event.attributes[0].list[0].genre_code).to.be.eq('ALL');
        expect(domainEvent.event.attributes[0].list[0].ranking.title).to.be.eq("WEEKLY RANKING");
